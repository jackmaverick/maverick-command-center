...
`invoiceReview` is the current active-JobNimbus-invoice total for every reviewed job. It must reconcile to the dashboard's invoiced-revenue total and is the revenue comparison basis for the live job-profit endpoint. It is billed revenue, not a cash claim. `cashReview` contains a separately verified timestamp, per-job applied/unapplied totals and payment-month totals, which must reconcile when present. A delayed or conflicting payment mirror retains the prior cash timestamp; it must not block a consistent invoice/profit publication. No raw payment objects, addresses or email bodies are exposed. `jobCostHolds` prevents a known reviewed discrepancy from being treated as reconciled until the next reviewer resolves it from current evidence.
